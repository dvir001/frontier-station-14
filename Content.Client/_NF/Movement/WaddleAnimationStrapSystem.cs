...
using Content.Shared._NF.Movement.Systems;
...
namespace Content.Client._NF.Movement.Systems;

public sealed class WaddleAnimationStrapSystem : SharedWaddleAnimationStrapSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeAllEvent<StartedWaddlingStrapEvent>(OnStartWaddling);
        SubscribeLocalEvent<WaddleAnimationStrapComponent, AnimationCompletedEvent>(OnAnimationCompleted);
        SubscribeAllEvent<StoppedWaddlingStrapEvent>(OnStopWaddling);
    }

    private void OnStartWaddling(StartedWaddlingStrapEvent msg, EntitySessionEventArgs args)
    {
        if (TryComp<WaddleAnimationStrapComponent>(GetEntity(msg.Entity), out var comp))
            StartWaddling((GetEntity(msg.Entity), comp));
    }

    private void OnStopWaddling(StoppedWaddlingStrapEvent msg, EntitySessionEventArgs args)
    {
        if (TryComp<WaddleAnimationStrapComponent>(GetEntity(msg.Entity), out var comp))
            StopWaddling((GetEntity(msg.Entity), comp));
    }

    private void StartWaddling(Entity<WaddleAnimationStrapComponent> entity)
    {
    ...
    private static float CalculateTumbleIntensity(WaddleAnimationStrapComponent component)
    {
        return component.LastStep ? 360 - component.TumbleIntensity : component.TumbleIntensity;
    }

    private static float CalculateAnimationLength(WaddleAnimationStrapComponent component, InputMoverComponent mover)
    {
        return mover.Sprinting ? component.AnimationLength * component.RunAnimationLengthMultiplier : component.AnimationLength;
    }

    private void OnAnimationCompleted(Entity<WaddleAnimationStrapComponent> entity, ref AnimationCompletedEvent args)
    {
    ...
    private void PlayWaddleAnimationStrapUsing(Entity<WaddleAnimationStrapComponent> entity, float len, float tumbleIntensity)
    {
