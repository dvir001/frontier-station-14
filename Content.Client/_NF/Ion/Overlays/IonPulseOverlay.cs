...
namespace Content.Client._NF.Ion.Overlays
{
    public sealed class IonPulseOverlay : Overlay
    {
        ...
        private readonly Dictionary<EntityUid, (ShaderInstance shd, IonShaderInstance instance)> _pulses = new();

        public IonPulseOverlay()
        {
            IoCManager.InjectDependencies(this);
            _baseShader = _prototypeManager.Index<ShaderPrototype>("Ion").Instance().Duplicate();
        }

        protected override bool BeforeDraw(in OverlayDrawArgs args)
        {
            IonQuery(args.Viewport.Eye);
        ...
        private void IonQuery(IEye? currentEye)
        {
            if (currentEye == null)
            {
                _pulses.Clear();
            ...
            var pulses = _entityManager.EntityQuery<IonPulseComponent>();
            ...
            foreach (var pulseEntity in activeShaderIds) //Remove all pulses that are added and no longer qualify
            {
                if (_entityManager.EntityExists(pulseEntity) &&
                    ...
                    _entityManager.TryGetComponent(pulseEntity, out IonPulseComponent? pulse))
                {
        ...
        private sealed record IonShaderInstance(MapCoordinates CurrentMapCoords, float Range, TimeSpan Start, float Duration)
        {
