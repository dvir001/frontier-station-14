...
namespace Content.Server._EE.Administration.Commands;
...
public sealed class PlayTimeUnlockCommands : IConsoleCommand
{
    ...
    public void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        var sys = _entManager.System<SharedRoleSystem>();
        var requirements = sys.GetJobRequirement(job!); // Add null-forgiving operator
        ...
        var jobPlaytimeRequirements = requirements
            .Where(r => r is OverallPlaytimeRequirement)
            .Cast<OverallPlaytimeRequirement>()
            .ToList();

        var jobDepartmentRequirements = requirements
            .Where(r => r is DepartmentTimeRequirement)
            .Cast<DepartmentTimeRequirement>()
            .ToList();
